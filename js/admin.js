...
async function handleLogin(event) {
  ...
  if (error) {
    loginError.textContent = "credenciales inválidas o cuenta no autorizada.";
    return;
  }
...
function renderCaptures(rows) {
  ...
  for (const row of rows) {
    ...
    img.alt = `captura del ${formatDate(row.created_at)}`;
    img.loading = "lazy";
...
async function loadCaptures() {
  ...
  if (error) {
    console.error("[Admin] Error cargando capturas:", error);
    return;
  }
  renderCaptures(data || []);
}
...
async function deleteCapture(row) {
  const confirmed = window.confirm("¿Eliminar esta captura permanentemente? Esta acción no se puede deshacer.");
  if (!confirmed) return;

  const { error: storageError } = await supabase.storage.from(BUCKET).remove([row.storage_path]);
  if (storageError) {
    console.warn("[Admin] No se pudo borrar el archivo del bucket:", storageError);
  }

  const { error: dbError } = await supabase.from(TABLE).delete().eq("id", row.id);
  if (dbError) {
    console.error("[Admin] No se pudo borrar la fila:", dbError);
    window.alert("No se pudo eliminar la captura. Revisa la consola para más detalles.");
    return;
  }
...
async function init() {
  if (!isSupabaseConfigured()) {
    loginError.textContent =
      "Supabase no está configurado todavía. Rellena js/supabase-config.js.";
    return;
  }
