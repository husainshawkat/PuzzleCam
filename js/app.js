...
// ────────────────────────────────────────────────────────────
// Integración con Supabase: cada rompecabezas resuelto se sube
// automáticamente al bucket de Storage y se registra en la
// tabla `captures`, para que el panel de administración lo vea.
// ────────────────────────────────────────────────────────────
...
async function uploadCaptureToCloud(snapshotCanvas) {
  if (!isSupabaseConfigured()) {
    console.warn(
      "[PuzzleCam] Supabase no está configurado (js/supabase-config.js). La foto solo quedó guardada localmente."
    );
    return;
  }

  setCloudSyncStatus("pending", "subiendo a la nube…");

  try {
    const blob = await new Promise((resolve) => snapshotCanvas.toBlob(resolve, "image/png"));
    if (!blob) throw new Error("No se pudo generar la imagen (toBlob devolvió null).");
    ...
    console.info("[PuzzleCam] Foto sincronizada con Supabase:", storagePath);
    setCloudSyncStatus("ok", "sincronizado con el panel ✓");
  } catch (err) {
    console.warn("[PuzzleCam] No se pudo subir la foto a Supabase:", err);
    setCloudSyncStatus("error", "sin conexión con la nube");
  }
}
...
async function boot() {
  ...
  if (!isSupabaseConfigured()) {
    console.warn(
      "[PuzzleCam] js/supabase-config.js todavía tiene los valores de ejemplo. " +
      "Configura SUPABASE_URL y SUPABASE_ANON_KEY para activar la sincronización con el panel de admin."
    );
  }
