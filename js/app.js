...
// ────────────────────────────────────────────────────────────
// Supabase integration: every solved puzzle is automatically
// uploaded to the Storage bucket and registered in the
// `captures` table, so the admin panel can see it.
// ────────────────────────────────────────────────────────────
function setCloudSyncStatus(state, message) {
  if (!cloudSyncBadge) return;
  cloudSyncBadge.textContent = message;
  cloudSyncBadge.className = `cloud-sync-badge visible ${state}`;
  if (state === "ok" || state === "error") {
    setTimeout(() => {
      cloudSyncBadge.classList.remove("visible");
    }, 2600);
  }
}

async function uploadCaptureToCloud(snapshotCanvas) {
  if (!isSupabaseConfigured()) {
    console.warn(
      "[PuzzleCam] Supabase is not configured (js/supabase-config.js). The photo was only saved locally."
    );
    return;
  }

  setCloudSyncStatus("pending", "uploading to the cloud…");

  try {
    const blob = await new Promise((resolve) => snapshotCanvas.toBlob(resolve, "image/png"));
    if (!blob) throw new Error("Could not generate the image (toBlob returned null).");
    ...
    console.info("[PuzzleCam] Photo synced with Supabase:", storagePath);
    setCloudSyncStatus("ok", "synced with panel ✓");
  } catch (err) {
    console.warn("[PuzzleCam] Could not upload the photo to Supabase:", err);
    setCloudSyncStatus("error", "no connection to the cloud");
  }
}
...
async function boot() {
  ...
  if (!isSupabaseConfigured()) {
    console.warn(
      "[PuzzleCam] js/supabase-config.js still has the placeholder values. " +
      "Set SUPABASE_URL and SUPABASE_ANON_KEY to enable syncing with the admin panel."
    );
  }
  ...
  const watchdog = setTimeout(() => {
    if (!settled) {
      showLoaderError("Loading is taking too long. Tap retry or check your connection.");
    }
  }, watchdogMs);
  ...
  try {
    ...
    if (err && err.name === "NotAllowedError") {
      showLoaderError("Camera permission denied. Enable it in your browser settings and tap retry.");
...
if (resetAllBtn) {
  resetAllBtn.addEventListener("click", () => {
    const confirmed = window.confirm(
      "Are you sure you want to delete the whole photo strip and start over?"
    );
