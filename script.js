...
let mediaRecorder = null, recordedChunks = [], recording = false, recStartTime = 0, recInterval = null, composeInterval = null;
...
function takePhoto() {
  ...
  captureCanvas.toBlob(blob => {
    ...
    openPreview(URL.createObjectURL(blob), 'image');
    flashShutter();
  }, 'image/png');
}
...
function startRecording() {
  ...
  if (currentStream) currentStream.getAudioTracks().forEach(t => stream.addTrack(t));
  ...
  mediaRecorder.onstop = () => {
    ...
    lastBlob = blob; lastFileType = 'video'; lastExt = 'webm';
    openPreview(URL.createObjectURL(blob), 'video');
  };
  mediaRecorder.start();
  composeInterval = setInterval(composeFrame, 33);
  recording = true;
  recStartTime = Date.now();
  shutterBtn.classList.add('recording');
  document.getElementById('rec-chip').classList.remove('hidden');
...
function stopRecording() {
  ...
  clearInterval(composeInterval); clearInterval(recInterval);
  recording = false;
...
const previewWrap = document.getElementById('preview-media-wrap');
...
function openPreview(url, type) {
  ...
  el.src = url;
  if (type === 'video') { el.controls = true; el.autoplay = true; el.loop = true; }
  previewWrap.appendChild(el);
  previewModal.classList.remove('hidden');
  updateThumb(url, type);
}
...
document.getElementById('preview-close').addEventListener('click', () => previewModal.classList.add('hidden'));
...
document.getElementById('preview-save').addEventListener('click', async () => {
  ...
  try {
    await uploadCapture({ blob: lastBlob, fileType: lastFileType, ext: lastExt });
